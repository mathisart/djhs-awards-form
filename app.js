...
(function () {
  ...
  const safeShow = (el, show = true) => { if (el) el.style.display = show ? "" : "none"; };
  const safeClass = (el, add = [], remove = []) => {
    if (!el) return;
  ...
  function setBackendBanner(ok, subText) {
    if (ok) {
      safeText(topWarn, "後端連線成功");
      safeClass(topWarn, ["is-ok"], ["is-error"]);
      safeText(backendStatus, subText || "已載入最新資料。");
    } else {
      safeText(topWarn, "後端連線失敗");
      safeClass(topWarn, ["is-error"], ["is-ok"]);
      safeText(backendStatus, subText || "無法連線後端，請稍後重試。");
    }
  }
  ...
  async function loadList() {
    try {
      ...
      safeHtml(tbody, '<tr><td colspan="10" class="empty">無法連線後端</td></tr>');
      safeText(countEl, "0");
  ...
  function renderTable(list) {
    const kw = (q && q.value ? q.value : "").toLowerCase();
    ...
    safeHtml(tbody, rows || `<tr><td colspan="10" class="empty">尚無資料</td></tr>`);
    safeText(countEl, String(list?.length ?? 0));
    updateButtons();
  }
  ...
  tbody && tbody.addEventListener("change", (e) => {
    if (e.target.classList && e.target.classList.contains("rowchk")) updateButtons();
  });
  ...
  $("#refreshBtn") && $("#refreshBtn").addEventListener("click", loadList);
  q && q.addEventListener("input", () => renderTable(_cacheList));

  // ─────────────────────────────────────────────────────────────
  // 加入名單（用 URL-encoded，避免預檢）
  // ─────────────────────────────────────────────────────────────
  form && form.addEventListener("submit", async (e) => {
    ...
    try {
      const body = new URLSearchParams(payload); // 不加自訂 header
  ...
  const normalizeTail = s => String(s||"").trim().replace(/[，。．、：:；;]+$/g, "");
  ...
  const getCheckedRows = () =>
    $$(".rowchk:checked").map(chk => {
      const t = chk.closest("tr").children;
      return { 班級: t[2].innerText.trim(), 座號: t[3].innerText.trim(), 姓名: t[4].innerText.trim(), 事由: t[6].innerText.trim() };
    });
  ...
  function buildEmceePreviewHTML() {
    ...
    return `<h2 style="margin:0 0 8px;">頒獎典禮司儀稿（預覽）</h2>
  ...
  btnEmcee && btnEmcee.addEventListener("click", () => {
  ...
  // ─────────────────────────────────────────────────────────────
  // 產生敘獎單（URL-encoded，避免預檢）
  // ─────────────────────────────────────────────────────────────
  btnAward && btnAward.addEventListener("click", async () => {
    ...
    try {
      ...
      const html = `<h2 style="margin:0 0 8px;">敘獎單已建立</h2>
  ...
  function showModal(title, html, opt = {}) {
    ...
    safeHtml(docBody, html);
    safeShow(copyBtn, !!opt.copy);
    safeShow(openDocBtn, !!opt.doc);
    safeShow(openPdfBtn, !!opt.pdf);
    safeShow(downloadPdfBtn, !!opt.frontPdf);
    safeShow(backdrop, true);
  }
  $("#closeModal") && $("#closeModal").addEventListener("click", () => safeShow(backdrop, false));

  copyBtn && copyBtn.addEventListener("click", async () => {
    try {
      await navigator.clipboard.writeText(docBody ? docBody.innerText : "");
      toast("已複製");
    } catch { toast("無法複製"); }
  });
